  ...
  return (
    <div className="fixed inset-0 z-40 flex min-h-0 flex-col overflow-hidden bg-white shadow-xl md:inset-y-0 md:left-auto md:right-0 md:w-[540px] md:border-l" style={{ borderColor: 'var(--border)' }}>
      <div className="flex shrink-0 items-center justify-between border-b px-4 py-3" style={{ borderColor: 'var(--border)' }}>
        ...
        <button type="button" aria-label="关闭回复模板编辑器" onClick={onClose} style={{ minHeight: 44, minWidth: 44 }}><X size={16} /></button>
      </div>
      <div className="flex shrink-0 border-b md:hidden" style={{ borderColor: 'var(--border)' }}>
        {(['edit', 'preview'] as const).map((t) => <button type="button" key={t} className="flex-1 border-b-2 py-2.5 text-sm font-medium" style={{ borderBottomColor: tab === t ? 'var(--primary)' : 'transparent', color: tab === t ? 'var(--primary)' : '#6b7280' }} onClick={() => setTab(t)}>{t === 'edit' ? '编辑' : '预览'}</button>)}
      </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {VALID_VARS.map((v) => <button type="button" key={v} className="rounded border border-indigo-100 bg-indigo-50 px-2 py-1.5 font-mono text-xs text-indigo-600 hover:bg-indigo-100" style={{ minHeight: 36 }} onClick={() => setContent((prev) => prev + v)}>{v}</button>)}
              </div>
            </div>
            <Field label="模板内容">
      ...
      <div className="flex shrink-0 flex-col justify-end gap-2 border-t p-4 md:flex-row" style={{ borderColor: 'var(--border)' }}>
        ...
        <button type="button" className="flex items-center justify-center gap-1.5 rounded-xl px-4 py-3 text-sm font-medium text-white disabled:opacity-50 md:rounded-lg md:py-2" style={{ background: invalidVars.length > 0 ? '#9ca3af' : 'var(--primary)', minHeight: 44 }} disabled={invalidVars.length > 0 || createTemplate.isPending || updateTemplate.isPending} onClick={save}><CheckCircle size={13} />保存模板</button>
      </div>
    </div>
  )
}
    ...
    <div className="flex min-h-full flex-col">
      <div className="flex flex-1 flex-col gap-4 p-4 md:p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <button type="button" className="flex shrink-0 items-center gap-1.5 rounded-lg px-3 py-2.5 text-sm font-medium text-white" style={{ background: 'var(--primary)', minHeight: 44 }} onClick={() => setEditing(null)}><Plus size={14} />新建模板</button>
        </div>
        <div className="relative" style={{ maxWidth: 280 }}>
          <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" />
          <input aria-label="搜索模板" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索模板..." className="w-full rounded-lg border bg-white py-2.5 pl-8 pr-3 text-sm focus:outline-none" style={{ borderColor: 'var(--border)', minHeight: 44 }} />
        </div>
          ...
          {isLoading ? <div className="p-4 text-sm text-gray-500">正在加载模板...</div> : (
            <table className="w-full min-w-[640px] text-sm">
              ...
              <tbody>{filtered.map((t) => <tr key={t.id} className="border-b last:border-0 hover:bg-gray-50" style={{ borderColor: 'var(--border)' }}>
                ...
                <td className="px-4 py-3"><div className="flex gap-1"><button type="button" aria-label={`编辑模板 ${t.name}`} className="rounded-lg p-2 text-gray-400 hover:bg-indigo-50 hover:text-indigo-600" onClick={() => setEditing(t)}><Edit3 size={13} /></button><button type="button" aria-label={`删除模板 ${t.name}`} className="rounded-lg p-2 text-gray-400 hover:bg-red-50 hover:text-red-600" onClick={() => setDeleteId(t.id)}><Trash2 size={13} /></button></div></td>
              </tr>)}</tbody>
            </table>
          )}
        ...
        <div className="flex flex-col gap-3 md:hidden">
          {filtered.map((t) => <div key={t.id} className="rounded-xl border bg-white p-4" style={{ borderColor: 'var(--border)' }}><div className="flex justify-between gap-2"><div className="min-w-0"><div className="truncate font-medium text-gray-800">{t.name}</div><div className="mt-1 line-clamp-2 text-xs text-gray-400">{t.content}</div></div><StatusBadge status={t.status === 'active' ? '已启用' : '草稿'} /></div><div className="mt-3 flex gap-2"><button type="button" className="flex-1 rounded-xl border py-2.5 text-xs" style={{ borderColor: 'var(--border)' }} onClick={() => setEditing(t)}>编辑</button><button type="button" className="flex-1 rounded-xl border py-2.5 text-xs text-red-500" style={{ borderColor: '#fca5a5' }} onClick={() => setDeleteId(t.id)}>删除</button></div></div>)}
        </div>
      </div>
      {editing !== undefined && <><button type="button" className="fixed inset-0 z-30 bg-black/20" aria-label="关闭回复模板编辑器" onClick={() => setEditing(undefined)} /><TemplateEditor template={editing} onClose={() => setEditing(undefined)} /></>}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  ...
  return <div className="flex flex-col gap-1.5"><label htmlFor={id} className="text-xs font-medium text-gray-600">{label}</label>{control}</div>
}
