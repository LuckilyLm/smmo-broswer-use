...
function date(value?: string) {
  ...
  return Number.isNaN(parsed.getTime()) ? value : new Intl.DateTimeFormat('zh-CN', { dateStyle: 'medium' }).format(parsed)
}
...
function InviteDrawer({ canInviteAdmin, loading, onClose, onInvite }: InviteDrawerProps) {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="invite-title" className="fixed inset-0 z-50 flex min-h-0 flex-col overflow-hidden bg-white shadow-xl md:inset-y-0 md:right-0 md:left-auto md:w-[400px] md:border-l" style={{ borderColor: 'var(--border)' }}>
      <div className="flex shrink-0 items-center justify-between border-b px-5 py-4" style={{ borderColor: 'var(--border)' }}>
        ...
        <button aria-label="关闭邀请面板" className="p-2 text-gray-400 hover:text-gray-600" onClick={onClose}><X size={18} /></button>
      </div>
      <div className="flex flex-1 flex-col gap-4 p-5">
        <label className="flex flex-col gap-1.5 text-sm font-medium text-gray-700">
          邮箱地址 <span className="sr-only">必填</span>
          <input autoFocus type="email" value={email} placeholder="member@company.com" onChange={(event) => { setEmail(event.target.value); setError('') }} className="min-h-11 rounded-lg border px-3 py-2.5 text-sm font-normal focus:outline-none" style={{ borderColor: error ? '#ef4444' : 'var(--border)' }} />
      ...
      <div className="flex shrink-0 gap-2 border-t p-4" style={{ borderColor: 'var(--border)' }}>
        <button className="min-h-11 flex-1 rounded-lg border px-4 py-2.5 text-sm hover:bg-gray-50" style={{ borderColor: 'var(--border)' }} onClick={onClose} disabled={loading}>取消</button>
        <button className="flex min-h-11 flex-1 items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium text-white disabled:opacity-60" style={{ background: 'var(--primary)' }} disabled={!valid || loading} onClick={submit}>
    ...
    <div className="flex min-h-full flex-col">
      <div className="flex flex-1 flex-col gap-4 p-4 md:p-6">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          {canManage && <button className="flex min-h-11 items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium text-white hover:opacity-90" style={{ background: 'var(--primary)' }} onClick={() => setInviteOpen(true)}><UserPlus size={15} />邀请成员</button>}
        </div>
