...
class SaaSStorage:
    ...
    def update_by_id(self, table: str, item_id: str, data: dict[str, Any], *, tenant_id: str | None = None) -> dict[str, Any] | None:
        db_table = _table(table)
        ...
        return self.get_by_id(table, item_id, tenant_id=tenant_id)
    ...
    def get_by_id(self, table: str, item_id: str, *, tenant_id: str | None = None) -> dict[str, Any] | None:
        db_table = _table(table)
        criteria = [db_table.c.id == item_id]
        if tenant_id is not None and "tenant_id" in db_table.c:
            criteria.append(db_table.c.tenant_id == tenant_id)
    ...
    def upsert_lead(self, lead: dict[str, Any]) -> dict[str, Any]:
        ...
        payload = self._merge_existing_lead_payload(payload)
        with self.session_factory.begin() as session:
        ...
        with self.session_factory.begin() as session:
            ...
            for row in stale_rows:
                session.execute(
                    ...
                    .values(status="failed", finished_at=now, error_type="worker_lost", error_message="worker heartbeat expired", updated_at=now)
                )
                session.execute(
                    update(executions)
                    .where(and_(executions.c.id == row["execution_id"], executions.c.status == "running"))
                    .values(status="failed", stage="failed", finished_at=now, error_type="worker_lost", error_message="worker heartbeat expired", updated_at=now)
                )
        return len(stale_rows)

    def _merge_existing_lead_payload(self, payload: dict[str, Any]) -> dict[str, Any]:
