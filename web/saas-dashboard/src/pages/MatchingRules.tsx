  ...
  return (
    <div className="fixed inset-0 z-40 flex min-h-0 flex-col overflow-hidden bg-white shadow-xl md:inset-y-0 md:left-auto md:right-0 md:w-[500px] md:border-l" style={{ borderColor: 'var(--border)' }}>
      <div className="flex shrink-0 items-center justify-between border-b px-4 py-3" style={{ borderColor: 'var(--border)' }}>
        ...
        <button type="button" aria-label="关闭匹配规则编辑器" onClick={onClose} style={{ minHeight: 44, minWidth: 44 }}><X size={16} /></button>
      </div>
      <div className="flex flex-1 flex-col gap-4 overflow-y-auto p-4">
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="优先级"><input type="number" value={priority} min={1} onChange={(e) => { const value = e.target.valueAsNumber; if (Number.isFinite(value)) setPriority(value) }} className={inp} /></Field>
          ...
          <Field label="匹配内容" span={2}><input aria-label="匹配内容" value={pattern} onChange={(e) => setPattern(e.target.value)} placeholder="price, quote, supplier" className={inp} /></Field>
        </div>
        <div className="rounded-xl border p-4" style={{ background: '#f8f9fb', borderColor: 'var(--border)' }}>
          ...
          <textarea aria-label="规则测试评论" value={testComment} onChange={(e) => setTestComment(e.target.value)} rows={3} placeholder="输入一段评论内容..." className={`${inp} resize-none`} />
          <button type="button" className="mt-3 w-full rounded-lg px-4 py-2.5 text-sm font-medium text-white disabled:opacity-50" style={{ background: 'var(--primary)', minHeight: 44 }} disabled={!testComment || !pattern || testRule.isPending} onClick={runTest}>运行测试</button>
      ...
      <div className="flex shrink-0 flex-col justify-end gap-2 border-t p-4 md:flex-row" style={{ borderColor: 'var(--border)' }}>
        <button type="button" className="rounded-xl border px-4 py-3 text-sm hover:bg-gray-50 md:rounded-lg md:py-2" style={{ borderColor: 'var(--border)', minHeight: 44 }} onClick={onClose}>取消</button>
        <button type="button" className="rounded-xl px-4 py-3 text-sm font-medium text-white disabled:opacity-50 md:rounded-lg md:py-2" style={{ background: 'var(--primary)', minHeight: 44 }} disabled={!campaignId || !templateId || !name || !pattern || createRule.isPending || updateRule.isPending} onClick={save}>保存规则</button>
      </div>
    </div>
  )
}
    ...
    <div className="flex min-h-full flex-col">
      <div className="flex flex-1 flex-col gap-4 p-4 md:p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <button type="button" className="flex shrink-0 items-center gap-1.5 rounded-lg px-3 py-2.5 text-sm font-medium text-white" style={{ background: 'var(--primary)', minHeight: 44 }} onClick={() => setDrawerRule(null)}><Plus size={14} />新建规则</button>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <select aria-label="选择营销活动" value={campaignId} onChange={(e) => setCampaignId(e.target.value)} className={inp} style={{ maxWidth: 320, borderColor: 'var(--border)', minHeight: 44 }}>{campaigns.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
          <div className="relative" style={{ maxWidth: 280 }}>
            <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" />
            <input aria-label="搜索规则" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索规则..." className="w-full rounded-lg border bg-white py-2.5 pl-8 pr-3 text-sm focus:outline-none" style={{ borderColor: 'var(--border)', minHeight: 44 }} />
          </div>
          ...
          {isLoading ? <div className="p-4 text-sm text-gray-500">正在加载规则...</div> : (
            <table className="w-full min-w-[760px] text-sm">
              ...
              <tbody>{filtered.map((r) => <tr key={r.id} className="border-b last:border-0 hover:bg-gray-50" style={{ borderColor: 'var(--border)' }}>
                ...
                <td className="px-4 py-3"><div className="flex gap-1"><button type="button" aria-label={`编辑规则 ${r.name}`} className="rounded-lg p-2 text-gray-400 hover:bg-indigo-50 hover:text-indigo-600" onClick={() => setDrawerRule(r)}><Edit3 size={13} /></button><button type="button" aria-label={`删除规则 ${r.name}`} className="rounded-lg p-2 text-gray-400 hover:bg-red-50 hover:text-red-600" onClick={() => setDeleteId(r.id)}><Trash2 size={13} /></button></div></td>
              </tr>)}</tbody>
            </table>
          )}
        ...
        <div className="flex flex-col gap-3 md:hidden">
          {filtered.map((r) => <div key={r.id} className="rounded-xl border bg-white p-4" style={{ borderColor: 'var(--border)' }}><div className="flex justify-between gap-2"><div className="min-w-0"><div className="font-medium text-gray-800">{r.name}</div><div className="mt-1 truncate text-xs text-gray-500">{r.pattern || '—'}</div></div><StatusBadge status={r.status === 'active' ? '已启用' : '已停用'} /></div><div className="mt-3 flex gap-2"><button type="button" className="flex-1 rounded-xl border py-2.5 text-xs" style={{ borderColor: 'var(--border)' }} onClick={() => setDrawerRule(r)}>编辑</button><button type="button" className="flex-1 rounded-xl border py-2.5 text-xs text-red-500" style={{ borderColor: '#fca5a5' }} onClick={() => setDeleteId(r.id)}>删除</button></div></div>)}
        </div>
      </div>
      {drawerRule !== undefined && <><button type="button" className="fixed inset-0 z-30 bg-black/20" aria-label="关闭匹配规则编辑器" onClick={() => setDrawerRule(undefined)} /><RuleDrawer rule={drawerRule} defaultCampaignId={campaignId} onClose={() => setDrawerRule(undefined)} /></>}
...
function Field({ label, children, span }: { label: string; children: React.ReactNode; span?: number }) {
  ...
  return <div className={`flex flex-col gap-1${span === 2 ? ' col-span-2' : ''}`}><label htmlFor={id} className="text-xs font-medium text-gray-600">{label}</label>{control}</div>
}
