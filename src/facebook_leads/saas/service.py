...
class SaaSService:
    ...
    def login(self, email: str, password: str) -> dict[str, Any]:
        ...
        self.storage.insert("sessions", {"id": token, "user_id": user["id"], "tenant_id": membership["tenant_id"]})
    ...
    def context_from_token(self, token: str) -> TenantContext:
        ...
        if not membership:
            raise PermissionError("tenant membership missing")
    ...
    def create_platform_account(self, context: TenantContext, data: dict[str, Any]) -> dict[str, Any]:
        ...
        return self.storage.insert("platform_accounts", {**payload, "tenant_id": context.tenant_id, "config_json": {}, "connection_metadata": {}, "login_status": "unknown"})
    ...
    def list_campaigns(self, context: TenantContext) -> list[dict[str, Any]]:
        campaigns = self.storage.list("campaigns", tenant_id=context.tenant_id)
    ...
    def create_campaign(self, context: TenantContext, data: dict[str, Any]) -> dict[str, Any]:
        ...
        return self.storage.insert("campaigns", {**defaults, **payload, "tenant_id": context.tenant_id})
    ...
    def delete_campaign(self, context: TenantContext, campaign_id: str) -> None:
        self.storage.delete_by_id("campaigns", campaign_id, tenant_id=context.tenant_id)
    ...
    def list_leads(self, context: TenantContext, filters: dict[str, Any] | None = None, *, limit: int = 50, offset: int = 0) -> dict[str, Any]:
        allowed = {"campaign_id", "platform", "status", "rule_intent_level", "reply_allowed", "keyword"}
        rows = self.storage.list("leads", tenant_id=context.tenant_id, filters={k: v for k, v in (filters or {}).items() if k in allowed}, limit=limit, offset=offset)
    ...
    def list_executions(self, context: TenantContext) -> list[dict[str, Any]]:
        return self.storage.list("executions", tenant_id=context.tenant_id)
    ...
    def cancel_execution(self, context: TenantContext, execution_id: str) -> dict[str, Any]:
        ...
        return self.storage.update_by_id("executions", execution_id, {"cancel_requested": True}, tenant_id=context.tenant_id) or execution
    ...
    def dashboard_summary(self, context: TenantContext) -> dict[str, Any]:
        ...
        return {
            ...
            "high_intent_leads": self._count(context, "leads", {"rule_intent_level": "high"}),
            ...
            "recent_executions": self.list_executions(context)[:5],
    ...
    def token_usage_details(self, context: TenantContext) -> dict[str, Any]:
        ...
        return {"by_model": by_model, "by_campaign": by_campaign}
    ...
    def enqueue_campaign_execution(self, context: TenantContext, campaign_id: str, *, trigger_type: str, schedule_trigger_key: str | None = None) -> dict[str, Any]:
        ...
        if queue is None:
            self.storage.update_by_id("executions", execution["id"], {"status": "cancelled", "stage": "duplicate_schedule"}, tenant_id=context.tenant_id)
    ...
    async def run_queue_item(self, queue_item: dict[str, Any]) -> dict[str, Any]:
        ...
        keywords = self.storage.list("campaign_keywords", tenant_id=context.tenant_id, filters={"campaign_id": campaign["id"], "enabled": True}, limit=1000, order_by=["priority", "created_at"])
        if not keywords:
        ...
        try:
            ...
            for index, keyword in enumerate(keywords, start=1):
                ...
                try:
                    ...
                    if status == "completed":
                        completed += 1
                        persist_orchestrator_result(self.storage, tenant_id=context.tenant_id, campaign_id=campaign["id"], platform_account_id=account["id"], platform=account["platform"], result=result, execution_id=execution["id"], execution_keyword_id=keyword_row["id"], keyword=keyword["keyword"])
                    else:
                        failed += 1
            ...
            if completed == 0 and failed == len(keywords) and retryable_failures == failed:
                for row in self.storage.list("execution_keywords", tenant_id=context.tenant_id, filters={"execution_id": execution["id"]}, limit=1000):
    ...
    def _update_execution_aggregate(self, context: TenantContext, execution_id: str, *, total: int, completed: int, failed: int, status: str | None = None, finished: bool = False) -> dict[str, Any] | None:
        ...
        return self.storage.update_by_id("executions", execution_id, payload, tenant_id=context.tenant_id)
    ...
    def _finish_queue_failure(self, queue_item: dict[str, Any], execution: dict[str, Any], error_type: str, message: str) -> dict[str, Any]:
        now = utc_now()
        self.storage.update_by_id("executions", execution["id"], {"status": "failed", "stage": "failed", "error_type": error_type, "error_message": message, "finished_at": now, "progress_percent": 100}, tenant_id=execution["tenant_id"])
