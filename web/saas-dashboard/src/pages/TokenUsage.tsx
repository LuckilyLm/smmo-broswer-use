...
export default function TokenUsage() {
  ...
  const dailyData = useMemo(() => {
    ...
    for (const item of details) {
      ...
      const row = map.get(key) || { day: key, input: 0, output: 0, total: 0 }
      row.input += item.prompt_tokens
      row.output += item.completion_tokens
      row.total += item.tokens_used
      map.set(key, row)
    }
  ...
  const byCampaign = useMemo(() => {
    ...
    for (const item of details) map.set(item.campaign_id || '未关联活动', (map.get(item.campaign_id || '未关联活动') || 0) + item.tokens_used)
    ...
    <div className="flex min-h-full flex-col">
      <div className="flex flex-1 flex-col gap-5 p-4 md:p-6">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <button className="flex items-center gap-1.5 rounded-lg border px-3 py-2 text-sm hover:bg-gray-50" style={{ borderColor: 'var(--border)', minHeight: 44 }} onClick={() => window.print()}><Download size={14} />导出</button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          {[
            ...
            { label: '调用次数', value: detailsPage?.total?.toLocaleString() || '0', icon: <Zap size={14} />, sub: '明细记录数' },
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className="rounded-xl border bg-white p-4" style={{ borderColor: 'var(--border)' }}>
            ...
            <ResponsiveContainer width="100%" height={180}>{dailyData.length ? <LineChart data={dailyData} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" vertical={false} /><XAxis dataKey="day" tick={{ fontSize: 11, fill: '#9ca3af' }} axisLine={false} tickLine={false} /><YAxis tick={{ fontSize: 11, fill: '#9ca3af' }} axisLine={false} tickLine={false} /><Tooltip contentStyle={{ fontSize: 12, borderRadius: 8 }} formatter={(v: any) => Number(v).toLocaleString()} /><Line type="monotone" dataKey="input" stroke="#4338ca" strokeWidth={2} dot name="输入" /><Line type="monotone" dataKey="output" stroke="#10b981" strokeWidth={2} dot name="输出" /></LineChart> : <EmptyChart />}</ResponsiveContainer>
          </div>
          <div className="rounded-xl border bg-white p-4" style={{ borderColor: 'var(--border)' }}>
            ...
            <ResponsiveContainer width="100%" height={180}>{byCampaign.length ? <BarChart data={byCampaign} layout="vertical" margin={{ top: 4, right: 8, bottom: 0, left: 0 }}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" horizontal={false} /><XAxis type="number" tick={{ fontSize: 11, fill: '#9ca3af' }} axisLine={false} tickLine={false} /><YAxis type="category" dataKey="campaign" tick={{ fontSize: 11, fill: '#6b7280' }} axisLine={false} tickLine={false} width={110} /><Tooltip contentStyle={{ fontSize: 12, borderRadius: 8 }} formatter={(v: any) => Number(v).toLocaleString()} /><Bar dataKey="tokens" fill="#4338ca" radius={[0, 4, 4, 0]} name="Token" /></BarChart> : <EmptyChart />}</ResponsiveContainer>
          </div>
        </div>
...
function EmptyChart() {
  return <div className="flex h-full items-center justify-center text-sm text-gray-400">暂无数据</div>
}
