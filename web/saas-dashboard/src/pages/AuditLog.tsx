  ...
  return (
    <div className="fixed inset-0 z-50 flex min-h-0 flex-col overflow-hidden bg-white shadow-xl md:inset-y-0 md:left-auto md:right-0 md:w-[480px] md:border-l" style={{ borderColor: 'var(--border)' }}>
      <div className="flex shrink-0 items-center justify-between border-b px-5 py-4" style={{ borderColor: 'var(--border)' }}>
        ...
        <button type="button" aria-label="关闭审计详情" onClick={onClose} style={{ minHeight: 44, minWidth: 44 }}><X size={16} /></button>
      </div>
      ...
      <div className="flex flex-1 flex-col gap-4 p-4 md:p-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative min-w-0 flex-1" style={{ maxWidth: 320 }}>
            ...
            <input aria-label="搜索审计日志" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索用户、操作或资源..." className="w-full rounded-lg border bg-white py-2.5 pl-8 pr-3 text-sm focus:outline-none" style={{ borderColor: 'var(--border)', minHeight: 44 }} />
          </div>
        ...
        <div className="hidden overflow-hidden rounded-xl border bg-white md:block" style={{ borderColor: 'var(--border)' }}>
          {isLoading ? <div className="p-4 text-sm text-gray-500">正在加载审计日志...</div> : <div className="overflow-x-auto"><table className="w-full min-w-[820px] text-sm"><thead><tr className="border-b bg-gray-50" style={{ borderColor: 'var(--border)' }}>{['时间', '用户', '操作', '资源', '结果', 'IP', ''].map((h) => <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-gray-500">{h}</th>)}</tr></thead><tbody>{filtered.map((log) => <tr key={log.id} className="cursor-pointer border-b last:border-0 hover:bg-gray-50" style={{ borderColor: 'var(--border)' }} tabIndex={0} onClick={() => setSelectedLog(log)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') setSelectedLog(log) }}><td className="px-4 py-3 text-xs text-gray-500">{formatDate(logTime(log))}</td><td className="px-4 py-3 text-xs text-gray-700">{log.user_display_name || log.user_email || log.user_id || '—'}</td><td className="px-4 py-3 font-medium text-gray-800">{log.action}</td><td className="px-4 py-3 font-mono text-xs text-gray-500">{log.resource_type || '—'} / {log.resource_id || '—'}</td><td className="px-4 py-3"><StatusBadge status={resultLabel(log.result)} /></td><td className="px-4 py-3 text-xs text-gray-400">{log.ip_address || '—'}</td><td className="px-4 py-3 text-xs text-indigo-600">查看</td></tr>)}</tbody></table>{filtered.length === 0 && <div className="p-8 text-center text-sm text-gray-400">暂无审计日志</div>}</div>}
        </div>
        <div className="flex flex-col gap-2 md:hidden">{filtered.map((log) => <button type="button"key={log.id} className="rounded-xl border bg-white p-4 text-left" style={{ borderColor: 'var(--border)' }} onClick={() => setSelectedLog(log)}><div className="flex justify-between gap-2"><div className="font-medium text-gray-900">{log.action}</div><StatusBadge status={resultLabel(log.result)} /></div><div className="mt-1 text-xs text-gray-400">{formatDate(logTime(log))} · {log.user_display_name || log.user_email || log.user_id || '—'}</div></button>)}</div>
      </div>
      {selectedLog && <><button type="button" className="fixed inset-0 z-40 bg-black/20" aria-label="关闭审计详情" onClick={() => setSelectedLog(null)} /><DetailDrawer log={selectedLog} onClose={() => setSelectedLog(null)} /></>}
    </div>
  )
}
