...
export default function LeadsInbox() {
  ...
  const effectiveSelectedId = useMemo(() => {
    if (selectedId && leads.some((lead) => lead.id === selectedId)) return selectedId
    return leads[0]?.id ?? null
  }, [leads, selectedId])
  const { data: selected, isLoading: selectedLoading, error: selectedError } = useLead(effectiveSelectedId ?? '')
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
      {mobileView === 'detail' && <div className="flex items-center gap-2 border-b bg-white px-4 py-2 md:hidden" style={{ borderColor: 'var(--border)' }}><button type="button" className="flex items-center gap-1.5 text-sm font-medium" style={{ color: 'var(--primary)', minHeight: 44 }} onClick={() => setMobileView('list')}><ArrowLeft size={16} />返回列表</button></div>}
          ...
          <div className="flex h-full w-full flex-col md:w-80">
            <div className="flex shrink-0 flex-col gap-2 border-b p-3" style={{ borderColor: 'var(--border)' }}>
              <div className="relative">
                <label htmlFor="lead-search" className="sr-only">搜索线索</label>
                <Search size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" />
                <input id="lead-search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索线索..." className="w-full rounded-lg border py-2 pl-7 pr-3 text-xs focus:outline-none" style={{ borderColor: 'var(--border)', minHeight: 40 }} />
              </div>
              <div className="flex flex-wrap gap-1">
                {[['', '全部'], ['high', '高意向'], ['medium', '中意向'], ['low', '低意向'], ['none', '无意向']].map(([value, label]) => <button type="button" key={value} className="rounded-lg px-2.5 py-1.5 text-xs" style={{ background: intentFilter === value ? 'var(--primary)' : '#f3f4f6', color: intentFilter === value ? 'white' : '#6b7280', minHeight: 36 }} onClick={() => setIntentFilter(value)}>{label}</button>)}
              </div>
            </div>
            <div className="min-h-0 flex-1 overflow-y-auto">
              ...
              {leads.map((lead) => <LeadRow key={lead.id} lead={lead} active={lead.id === effectiveSelectedId} onClick={() => choose(lead.id)} />)}
            </div>
          </div>
        </div>
            ...
            <>
              ...
              <div className="min-h-0 flex-1 overflow-y-auto p-4">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <button type="button" className="flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm text-white disabled:opacity-50" style={{ background: 'var(--primary)', minHeight: 44 }} disabled={contacted.isPending} onClick={() => contacted.mutate(selected.id)}><CheckCircle size={14} />标记已联系</button>
                  <button type="button" className="flex items-center gap-1.5 rounded-lg border px-3 py-2 text-sm text-red-500 disabled:opacity-50" style={{ borderColor: '#fca5a5', minHeight: 44 }} disabled={invalid.isPending} onClick={() => invalid.mutate({ id: selected.id, reason: '人工标记无效' })}><XCircle size={14} />标记无效</button>
                </div>
              </div>
            </>
          )}
...
function LeadRow({ lead, active, onClick }: { lead: Lead; active: boolean; onClick: () => void }) {
  return <button type="button" className="w-full border-b p-3 text-left hover:bg-gray-50" style={{ borderColor: 'var(--border)', background: active ? 'var(--accent)' : 'white' }} onClick={onClick}><div className="flex items-start justify-between gap-2"><div className="min-w-0"><div className="truncate text-sm font-medium text-gray-900">{lead.author_name || '未知作者'}</div><div className="mt-1 line-clamp-2 text-xs text-gray-500">{lead.comment_text || '—'}</div></div><StatusBadge status={intentLabel(lead.final_intent_level)} /></div><div className="mt-2 flex items-center justify-between text-[11px] text-gray-400"><span>{lead.platform}</span><span>{formatDate(lead.created_at)}</span></div></button>
}
